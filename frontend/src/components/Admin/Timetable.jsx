...
const Timetable = () => {
  ...
  const fetchTimetable = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://lms-ul7x.onrender.com/api/admin/timetable/${selectedClass}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://lms-ul7x.onrender.com/api/admin/timetable', {
        method: 'POST',
        headers: {
